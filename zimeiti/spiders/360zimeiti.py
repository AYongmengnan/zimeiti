...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.360zimeiti.com/"]
    path = f'D:/gushi365/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@id="nav-industry"]/li/a')
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//ul[@class="specificpost-list"]/li/div[1]/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        title = response.xpath('//div[@id="post_title"]/text()').get()
        if title:
            item['title'] = title
        content = response.xpath('//div[@id="post_description"]/*').getall()
        if content:
            text = "".join(content)
...
if __name__ == '__main__':
    os.system('scrapy crawl 360zimeiti')