...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://ju.51tietu.net/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="nav"]/a')[1:]
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="textb"]/div/ul/li/h2/a/@href').getall()
        if lists:
            for li in lists:
                ...
                if num == 0:
                    ...
                    print('数据库已存在')
                    pass
        next_page = repsonse.xpath('//div[@class="p"]/a[@class="next"]/@href').get()
        if next_page:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="sub-left-content"]/div/h1/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[@id="pic"]/div/*').getall()
        if content:
            text = "".join(content)
...
if __name__ == '__main__':
    os.system('scrapy crawl 51tietu')