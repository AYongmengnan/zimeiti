...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.8080177.cn/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="nav"]/ul/li/a/@href').getall()
        if lanmus:
            for lmu in lanmus:
                lmurl = urljoin(response.url, lmu.xpath('@href').get())
                yield scrapy.Request(url=lmurl,callback=self.ejlm,meta={'ncolumn':ncolumn})

    def ejlm(self, response):
        lanmus = response.xpath('//div[@class="nav"]/ul/li/a')
        if lanmus:
            for lmu in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="list"]/ul/li/h6/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//h1[@class="title"]/strong/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[@id="content"]').getall()
        if content:
            text = "".join(content)
...
if __name__ == '__main__':
    os.system('scrapy crawl 8080177')