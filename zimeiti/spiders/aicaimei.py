...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.aicaimei.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="wrapin"]/a')[1:]
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="g-newsbox"]/ul/li/a')
        if lists:
            for li in lists:
                fmt = li.xpath('i/img/@src').get()
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//h1[@class="t1"]/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[@class="content"]/p').getall()
        if content:
            text = "".join(content)
...
if __name__ == '__main__':
    os.system('scrapy crawl aicaimei')