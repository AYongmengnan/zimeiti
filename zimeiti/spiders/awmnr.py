...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.awmnr.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@id="tab_menu"]/li/a')[1:]
        print(lanmus)
        if lanmus:
            for lmu in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="colunmcont"]/ul/li/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="topmain"]/h1/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[@class="topmain"]/*[not(name()="h1") and not(name()="div")]').getall()
        if content:
            text = "".join(content)
