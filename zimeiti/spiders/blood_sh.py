...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.blood-sh.com.cn/"]
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="articles"]/ul/li/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="article-wrapper"]/h1/text()').get()
        if item['title']:
...
if __name__ == '__main__':
    os.system('scrapy crawl blood_sh')