...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.cjmx.com/"]
    path = f'D:/gushi365/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="hide_nav"]')
        lanmu_lists = []
        if lanmus:
            for lm in lanmus:
                lumn = lm.xpath('a')[1:]
                lanmu_lists += lumn
        if lanmu_lists:
            for lls in lanmu_lists:
                # print(ncolumn)
                ncolumn = lls.xpath('text()').get().strip()
                lanmu_url = urljoin(self.start_urls[0],lls.xpath('@href').get())
                # print(lanmu_url)
                yield scrapy.Request(url=lanmu_url,callback=self.lists,meta={'ncolumn':ncolumn})
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="products"]/p/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        title = response.xpath('//p[@class="conl_wz_b"]/text()').get()
        if title:
            item['title'] = title
        content = response.xpath('//div[@class="artconpp"]/*').getall()
        if content:
            text = "".join(content)
