...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.eweb.net.cn/jiangu/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="bm_c"]/ul/li/a')
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="bm_c xld"]/dl')
        if lists:
            for li in lists:
                img_li = li.xpath('dd/div/a')
                if img_li:
                    fmt = img_li.xpath('img/@src').get()
                    fmt_url = urljoin(self.start_urls[0],fmt)
                # print(fmt_url)
                    de_url = img_li.xpath('@href').get()
                else:
                    de_url = li.xpath('dt/a/@href').get()
                    fmt_url = None
                ...
                if num == 0:
                    ...
                    print('数据库已存在')
                    pass
        next_page = repsonse.xpath('//div[@class="pg"]/a[contains(text(),"下一页")]/@href').get()
        if next_page:
    ...
    def detail(self,response):
        ...
        content = response.xpath('//td[@id="article_content"]').getall()
        if content:
            text = "".join(content)
            item['Ncontent'] = refactoring_img(text,response.url,self.path)
            # item['Ncontent'] = refactoring_img(text,response.url,self.path)
            # item['Ncontent'] = content
            item['description'] = response.xpath('//div[@class="s"]/div/text()').get().replace(':','')
