...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.faxingzhan.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="nav"]/ul/li')[1:-1]
        lmurls = []
        if lanmus:
            for lm in lanmus:
                small = lm.xpath('ul/li/a')
                if small:
                    lmurls += small
                else:
                    lmurls.append(lm.xpath('a'))
        # print(len(lmurls),lmurls)
        zhuanti = response.xpath('//div[@class="cf-content"]/ul/li/a')
        lmurls += zhuanti
        if lmurls:
            for lmu in lmurls:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="nList"]/ul/li/dl/dt/a')
        if lists:
            for li in lists:
