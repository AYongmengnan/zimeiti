...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.gimipet.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="nav-fl"]/a')[1:4]
        print(lanmus)
        if lanmus:
            for lmu in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//ul[@class="list-ul"]/li/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[contains(@class,"ptitle")]/h1/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[contains(@class,"txt overflow")]').getall()
        if content:
            text = "".join(content)
...
if __name__ == '__main__':
    os.system('scrapy crawl gimipet')