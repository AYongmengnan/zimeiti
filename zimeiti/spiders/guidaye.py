...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.guidaye.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@id="starlist"]/li/ul/li/a')
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//ul[@id="post_list_box"]/li/i/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="newsview"]/h1/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[@id="artcont"]/div/*').getall()
        if content:
            text = "".join(content)
...
if __name__ == '__main__':
    os.system('scrapy crawl guidaye')