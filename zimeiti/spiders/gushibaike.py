...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.gushibaike.net/baike"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@class="navul"]/li/a/@href').getall()[1:]
        if lanmus:
            for lm in lanmus:
                lanmu_url = urljoin(self.start_urls[0],lm)
                # print(lanmu_url)
                time.sleep(random.uniform(3,5))
                yield scrapy.Request(url=lanmu_url,callback=self.ejlm)
    def ejlm(self, response):
        lanmus = response.xpath('//ul[@class="cateul"]/li/a')
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="section-list"]/ul/li/a/@href').getall()
        if lists:
            for li in lists:
                ...
                if num == 0:
                    ...
                    print('数据库已存在')
                    pass
        next_page = repsonse.xpath('//div[@class="pages"]/a[contains(text(),"下页")]/@href').get()
        if next_page:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        title = response.xpath('//h1[@class="htitle"]/text()').get()
        if title:
            item['title'] = title
        content = response.xpath('//div[@class="con-txt"]').getall()
        if content:
            text = "".join(content)
...
if __name__ == '__main__':
    os.system('scrapy crawl gushibaike')