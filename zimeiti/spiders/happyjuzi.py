...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.happyjuzi.com/"]
    path = f'D:/gushi365/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@class="nav_header_0"]/li/a')[1:2] # 去掉首页和vip
        if lanmus:
            for lm in lanmus:
                ncolumn = lm.xpath('text()').get()
                if ncolumn:
                    ncolumn = ncolumn.strip()
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="pc-item"]/a')
        if lists:
            for li in lists:
                ...
                if num == 0:
                    ...
                    print('数据库已存在')
                    pass
        next_page = repsonse.xpath('//a[@class="next"]/@href').get()
        if next_page:
    ...
    def detail(self,response):
        ...
        if title:
            item['title'] = title
        content = response.xpath('//div[@class="detail-content"]').getall()
        if content:
            text = "".join(content)
            ...
            item['tag'] = '#'.join(response.xpath('//div[@class="item-tag"]/a/text()').getall())
...
if __name__ == '__main__':
    os.system('scrapy crawl happyjuzi')