...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.idafoo.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@id="nav_links"]/li/a')[1:]
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="list_item"]/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="viewtitle"]/h1/text()').get()
        if item['title']:
