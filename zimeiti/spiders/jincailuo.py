...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.jincailuo.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="header"]/div/ul/li/a')
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//ul[@class="list"]/li/div[@class="img"]/a')
        if lists:
            for li in lists:
...
if __name__ == '__main__':
    os.system('scrapy crawl jincailuo')