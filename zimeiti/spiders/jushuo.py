...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.jushuo.com/"]
    path = f'D:/gushi365/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="nav2_list"]/a')
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="new_list"]/ul/li/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        if title:
            item['title'] = title
        content = response.xpath('//div[@class="detail_main"]/*').getall()
        if content:
            text = "".join(content)
