...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.minghaoshangwu.com/meirongpindao/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@class="nav"]/li')[1:]
        lmurls = []
        if lanmus:
            for lm in lanmus:
                small = lm.xpath('dl/dd/a')
                if small:
                    lmurls += small
                else:
                    lmurls.append(lm.xpath('strong/a'))
        # print(len(lmurls),lmurls)
        if lmurls:
            for lmu in lmurls:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[@class="listl list2"]/ul/li/div/a[2]')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="listltitle"]/h1/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[contains(@class,"article-content")]').getall()
        if content:
            text = "".join(content)
            item['Ncontent'] = refactoring_img(text,response.url,self.path)
            # item['Ncontent'] = refactoring_img(text,response.url,self.path)
            # item['Ncontent'] = content
            item['description'] = response.xpath('//div[@class="say"]/p/text()').get()
            item['nkeywords'] = get_words(item['Ncontent'])
            item['tag'] = response.xpath('//div[@class="tag"]/a/text()').get()
...
if __name__ == '__main__':
    os.system('scrapy crawl minghaoshangwu')