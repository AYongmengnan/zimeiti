...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.mingzihui.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="nav1"]/div/ul/li/a')
        if lanmus:
            for lm in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//div[contains(@class,"fl over0")]/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//h2[@class="articletitle"]/text()').get()
        if item['title']:
...
if __name__ == '__main__':
    os.system('scrapy crawl mingzihui')