...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.sanwendaquan.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//ul[@class="nav_wrap"]/li/span/a/@href').getall()[1:]
        if lanmus:
            for lmu in lanmus:
    ...
    def erjilm(self, response):
        lanmus = response.xpath('//div[@class="con_lm"]/div//h3/a')
        if lanmus:
            for lmu in lanmus:
...
if __name__ == '__main__':
    os.system('scrapy crawl sanwendaquan')