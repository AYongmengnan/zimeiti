...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.souid.com/baike/index.html"]
    ...
    def parse(self,repsonse):
        lists = repsonse.xpath('//ul[@class="imgbox"]/a')
        # print(len(lists))
        if lists:
            for li in lists:
                ...
                if num == 0:
                    ...
                    print(imgUrl)
                    yield scrapy.Request(url=detail_url,callback=self.detail,meta={'ncolumn':'行业百科','imgUrl':imgUrl})
                else:
                    print('数据库已存在')
                    pass
        next_page = repsonse.xpath('//div[@class="page"]/a[contains(text(),"下一页")]/@href').get()
        if next_page:
            next_url = urljoin(repsonse.url,next_page)
            yield scrapy.Request(url=next_url,callback=self.parse,meta={'ncolumn':'行业百科'})
