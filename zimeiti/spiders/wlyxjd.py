...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.wlyxjd.com/shenghuo/list_5_1.html"]
    ...
    def parse(self, response, **kwargs):
        lists = response.xpath('//div[@class="posts-gallery-content"]/h2/a/@href').getall()
        # print(len(lists))
        if lists:
            for li in lists:
                ...
                if num == 0:
                    ...
                    # print(imgUrl)
                    yield scrapy.Request(url=detail_url,callback=self.detail,meta={'ncolumn':'生活题库'})
                else:
                    print('数据库已存在')
                    pass
        next_page = response.xpath('//div[@class="pages"]/ul/li/a[contains(text(),"下一页")]/@href').get()
        if next_page:
    ...
    def detail(self,response):
        print('获取内容')
        item = ZimeitiItem()
        title = response.xpath('//div[@class="art-con"]/h1/text()').get()
        if title:
            item['title'] = title
        content = response.xpath('//div[@id="cont"]/*').getall()
        if content:
            text = "".join(content)
