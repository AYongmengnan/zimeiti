...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.zfw88.com/"]
    path = f'//192.168.0.15/data/SEO/images/{name}/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="nav"]/ul/li/a/@href').getall()[1:-1]
        if lanmus:
            for lmu in lanmus:
    ...
    def erjilm(self, response):
        lanmus = response.xpath('//ul[@class="menu"]/li/a')
        if lanmus:
            for lmu in lanmus:
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//ul[@class="list2"]/li/a')
        if lists:
            for li in lists:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="g_con"]/h1/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[@class="con"]').getall()
        if content:
            text = "".join(content)
