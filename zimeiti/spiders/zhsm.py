...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.zhsm.net/gongzuofanwen/gongzuozongjie/"]
    path = f'//192.168.0.15/data/SEO/images/gdyjs/'
    s = 0
    l = 0
    def parse(self, response):
        lanmus = response.xpath('//div[@class="lm_title"]/a/@href').getall()
        if lanmus:
            for lm in lanmus:
                # ncolumn = lm.xpath('text()').get().strip()
                lmurl = urljoin(response.url,lm)
                yield scrapy.Request(url=lmurl,callback=self.lists)
    ...
    def lists(self,repsonse):
        lists = repsonse.xpath('//li[@class="list_itme2"]/a')
        if lists:
            for li in lists:
                ...
                num = is_exists({'name':'gdyjs','url':detail_url})
                ...
                if num == 0:
                    ...
                    print('数据库已存在')
                    pass
        next_page = repsonse.xpath('//div[@class="pnum"]/ul/a[contains(text(),"下一页")]/@href').get()
        if next_page:
    ...
    def detail(self,response):
        ...
        self.s += 1
        item = ZimeitiItem()
        item['title'] = response.xpath('//div[@class="ar_title"]/h1/text()').get()
        if item['title']:
            item['title'] = item['title'].strip()
        content = response.xpath('//div[@id="docopy"]/p').getall()
        if content:
            text = "".join(content)
